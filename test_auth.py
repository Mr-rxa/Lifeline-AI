...
# 2. Complete Setup
res = requests.post(f'{BASE_URL}/auth/setup/complete', json={'username': 'admin', 'email': 'admin@lifeline.ai', 'password': 'password123'})
...
res = requests.post(f'{BASE_URL}/users/', json={'username': 'dispatcher1', 'email': 'disp@lifeline.ai', 'password': 'pass', 'role': 'dispatcher'}, headers=headers)
print("Create User:", res.json())
assert res.status_code == 201

# 6. Verify Login works for new user
res = requests.post(f'{BASE_URL}/auth/login', json={'username': 'dispatcher1', 'password': 'pass'})
...
# 8. Password Reset
res = requests.post(f'{BASE_URL}/auth/reset-password', json={'username': 'dispatcher1', 'new_password': 'newpass'})
print("Password Reset:", res.json())
assert res.status_code == 200

# 9. Register via public endpoint
res = requests.post(f'{BASE_URL}/auth/register', json={'username': 'publicuser', 'email': 'pub@lifeline.ai', 'password': 'pub'})
